...
const BINARY = "/usr/local/bin/devm8"

describe("generatePlist", () => {
  it("contains the correct Label key", () => {
    const xml = generatePlist(BINARY)
    expect(xml).toContain("<key>Label</key>")
    expect(xml).toContain("<string>net.devm8</string>")
  })
...
describe("writePlist", () => {
  ...
  it("creates the plist file at PATHS.plistFile", async () => {
    ...
    expect(content).toContain("net.devm8")
...
describe("agentStatus", () => {
  ...
  it("falls back to launchctl list when print fails", async () => {
    const listOutput = "12345\t0\tnet.devm8\n"
    let callCount = 0
